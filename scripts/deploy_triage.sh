...
EXPLAINER_MODEL="${AKSHA_EXPLAINER_MODEL:-gemini-3.5-flash-lite}"
...
gcloud run deploy "${SERVICE}" \
  ...
  --memory 1Gi \
  --timeout 600 \
  --set-env-vars "GOOGLE_CLOUD_PROJECT=${PROJECT},GOOGLE_CLOUD_LOCATION=${MODEL_LOCATION},GOOGLE_GENAI_USE_ENTERPRISE=True,AKSHA_INVESTIGATOR_MODEL=${INVESTIGATOR_MODEL},AKSHA_EXPLAINER_MODEL=${EXPLAINER_MODEL}" \
  --quiet
